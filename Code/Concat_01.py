...
class MIMIC3(torch.utils.data.Dataset):
    def __init__(self, col_list="default", attr_list="default", categorize = None, data_opt = 'train', scaling = 'mean-std', holdout = 0.3, random_seed = 42, oversampler = None):
        ...
        self.datasetY['YN'] = 1
        self.datasetY['YN'][self.datasetY['DEATHTIME'].isna()] = 0
        # self.datasetY['YN'][self.datasetY['DEATHTIME'].] = 0
        print(self.datasetY)
        print(self.datasetY['DEATHTIME'].value_counts().sum())
        ...
        self.datasetY = self.datasetY.drop(['ADMITTIME', 'DISCHTIME', 'DEATHTIME'], axis=1)
        self.datasetY = self.datasetY.to_numpy()
        # 여기까지 뜯어 고쳐야함.
...
def test_ffnet(model, test_loader):
    ...
    with torch.no_grad():
        ...
        criterion2 = RMSELoss
        acc = accuracy_score(y, yhat.round())
        for i in preds:
            if i > 1:
                print(i)
